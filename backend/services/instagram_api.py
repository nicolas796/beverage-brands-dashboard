"""
Instagram API integration using RapidAPI (Instagram Looter by IRROR Systems)
Rate limits: 1000 req/hour, 150/month
"""
import os
import time
import json
...
INSTAGRAM_RATE_LIMIT_HOURLY = 1000
INSTAGRAM_RATE_LIMIT_MONTHLY = 150

# RapidAPI configuration
RAPIDAPI_KEY = os.getenv('RAPIDAPI_KEY', '29224a2606msh79c9dc264c77b53p1c3dcbjsn81eef776109f')
INSTAGRAM_API_HOST = "instagram-looter2.p.rapidapi.com"

# In-memory rate limit tracking
class RateLimiter:
    """Simple rate limiter for Instagram API"""
    
    def __init__(self, hourly_limit: int = 1000, monthly_limit: int = 150):
    ...
    def can_make_request(self) -> Tuple[bool, Optional[str]]:
        ...
        if len(self.requests_this_month) >= self.monthly_limit:
            return False, "Monthly rate limit exceeded (150 requests/month)"
...
instagram_rate_limiter = RateLimiter(INSTAGRAM_RATE_LIMIT_HOURLY, INSTAGRAM_RATE_LIMIT_MONTHLY)
...
class InstagramAPI:
    """Instagram API client using RapidAPI"""
    
    def __init__(self, api_key: Optional[str] = None):
        self.api_key = api_key or RAPIDAPI_KEY
        self.base_url = f"https://{INSTAGRAM_API_HOST}"
        self.headers = {
            "X-RapidAPI-Key": self.api_key,
            "X-RapidAPI-Host": INSTAGRAM_API_HOST,
            "Accept": "application/json"
        }
        self.rate_limiter = instagram_rate_limiter
    ...
    def _make_request(self, endpoint: str, params: Dict[str, Any] = None, retries: int = 2) -> Dict:
        ...
        if not can_request:
            raise InstagramAPIError(f"Rate limit exceeded: {error_msg}")
        ...
        for attempt in range(retries + 1):
            try:
                ...
                if response.status_code == 429:
                    raise InstagramAPIError("Rate limit exceeded (429 from API)")
                elif response.status_code == 404:
                    raise InstagramAPIError(f"User not found (404)")
                elif response.status_code == 401:
                    raise InstagramAPIError("Invalid API key (401)")
                ...
                if isinstance(data, dict):
                    if data.get('status') == 'error':
                        raise InstagramAPIError(f"API Error: {data.get('message', 'Unknown error')}")
                    if data.get('error'):
                        raise InstagramAPIError(f"API Error: {data.get('error')}")
        ...
        raise InstagramAPIError("Unexpected error in request")
    ...
    def get_user_info(self, username: str) -> InstagramUserInfo:
        ...
        Returns:
            InstagramUserInfo object with user data
            
        Raises:
            InstagramAPIError: If API request fails or user not found
        """
        # Remove @ if present
        ...
        # API endpoint for profile info (Instagram Looter API)
        ...
        try:
            ...
            # Parse response from Instagram Looter API
            # Response structure: {"data": {"user": {...}}}
            data_wrapper = data.get('data', {})
            user_data = data_wrapper.get('user', {})
            
            # Extract followers, following, posts from edge_* fields
            ...
            user_info = InstagramUserInfo(
                ...
                followers=self._parse_number(edge_followed_by.get('count', 0)),
                following=self._parse_number(edge_follow.get('count', 0)),
                posts=self._parse_number(edge_owner_to_timeline_media.get('count', 0)),
            ...
            logger.info(f"Successfully fetched Instagram user: {username} "
                       f"({user_info.followers:,} followers, {user_info.posts} posts)")
            ...
            raise InstagramAPIError(f"Failed to parse user data: {str(e)}")
...
def get_instagram_user(username: str, api_key: Optional[str] = None) -> Dict[str, Any]:
    """
    Convenience function to fetch Instagram user info
...
if __name__ == "__main__":
    ...
    result = get_instagram_user(test_username)
